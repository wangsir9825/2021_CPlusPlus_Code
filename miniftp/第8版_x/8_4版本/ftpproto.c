...
extern session_t *p_sess; //声明全局会话结构指针（在超时断开连接时，会用到这个指针）

void ftp_reply(session_t *sess, unsigned int code, const char *text) // FTP 回应方法
{
...
// 1 控制连接空闲断开（如果客户端不发送任何命令并且也不传输数据，就断开控制连接）
void handle_ctrl_timeout(int sig) // 自定义信号回调接口，sig是信号值
{
	//421 Timeout.
	shutdown(p_sess->ctrl_fd, SHUT_RD); // 关闭控制连接套接字的读端口
	ftp_reply(p_sess, FTP_IDLE_TIMEOUT, "Timeout."); // 发送421代码（提示超时）
	shutdown(p_sess->ctrl_fd, SHUT_WR); // 关闭控制连接套接字的写端口
	exit(EXIT_SUCCESS);
}

void start_cmdio_alarm() // 开始闹钟信号
{
	if(tunable_idle_session_timeout > 0) // 如果控制连接超时时间配置项的值大于0，则开始设置闹钟信号
	{
		signal(SIGALRM, handle_ctrl_timeout); // 将14号闹钟信号的处理方式设置为自定义方式
		alarm(tunable_idle_session_timeout); // 设置一个定时器，时间到了就会发送给当前进程一个SIGALRM信号（闹钟时间由配置项决定）
	}
}
...
{
	if(!p_sess->data_process) // 如果数据连接处于断开状态，则断开控制连接
	{
		//空闲断开
		ftp_reply(p_sess, FTP_DATA_TIMEOUT, "Data timeout, Reconnect Sorry."); // 发送421代码（数据连接超时）
		exit(EXIT_FAILURE);
	}
	p_sess->data_process = 0; // 将数据连接标记为断开状态
	start_data_alarm();  // 启动闹钟，即重新启动数据连接的空闲断开
}
void start_data_alarm()
{
	if(tunable_data_connection_timeout > 0) // 如果数据连接配置项的值大于0
	{
...
void handle_child(session_t *sess)//ftp 服务进程
{
    ...
    while(1)//不停的等待客户端的命令并做出处理
    {
			...
			//开启 控制连接空闲断开功能
			start_cmdio_alarm(); // 每次执行到这一句，闹钟时间都会从头开始（可以理解为重新启动闹钟）
...
void list_common(session_t *sess) // 发送文件列表数据
{
	...
	// 将文件信息组合起来，并保存在buf中
		...
		offset += sprintf(buf+offset, "%3d %-8d %-8d %8llu ",   // 
...
static void do_list(session_t *sess) // 显示资源列表
{
	...
	// 下载数据完毕后，会关闭控制连接闹钟，所以需要重新开启控制连接断开
	start_cmdio_alarm();
}
...
//////////////////////////////////////////////////////////////////////////////////
void limit_rate(session_t *sess, unsigned long bytes_transfer, int is_upload) // 限制数据传输速率
{
	//登记文件传输结束时间
	...
	pass_time += ((double)(cur_usec - sess->transfer_start_usec) / 1000000); // 微秒转换为秒

	//计算当前的数据传输速度
	unsigned long cur_rate = (unsigned long)(bytes_transfer / pass_time); // 当前传输速度 = 已传输字节数/传输时长
	double rate_ratio; //当前传输速度与最大传输速率配置项的比值

	if(is_upload) // 如果是上传数据
	{
		//上传
		if(tunable_upload_max_rate==0 || cur_rate<=tunable_upload_max_rate) // 如果配置文件中，最大上传速率配置项为0或当前速度小于最大上传速率配置项的设置值，则不限速
		{
			//不限速需要重新登记时间
		...
		rate_ratio = cur_rate / tunable_upload_max_rate; // 计算当前传输速度与最大上传速率配置项的比值
	}
	else // 如果是下载数据
	{
		//下载
		if(tunable_download_max_rate==0 || cur_rate <= tunable_download_max_rate) // 如果配置文件中，最大下载速率配置项为0或当前速度小于最大下载速率配置项的设置值，则不限速
		{
			//不限速需要重新登记时间
			sess->transfer_start_sec = get_time_sec();
			sess->transfer_start_usec = get_time_usec();
		...
		rate_ratio = cur_rate / tunable_download_max_rate; // 计算当前传输速度与最大下载速率配置项的比值
	}

	double sleep_time = (rate_ratio - 1) * pass_time; // 计算休眠时间
	
	nano_sleep(sleep_time); // 进入休眠

	//休眠结束，重新登记开始时间，为下一次休眠做准备
	sess->transfer_start_sec = get_time_sec();
	sess->transfer_start_usec = get_time_usec();
}
...
static void do_retr(session_t *sess) // 下载文件
{
	...
	if(sess->is_ascii) // 如果是ASCII模式传输，响应信息如下
		sprintf(buf, "Opening ASCII mode data connection for %s (%llu bytes).", sess->arg, (unsigned long long)sbuf.st_size); 
	else // 如果是二进制模式传输，响应信息如下
		sprintf(buf, "Opening BINARY mode data connection for %s (%llu bytes).",sess->arg, (unsigned long long)sbuf.st_size);
	ftp_reply(sess, FTP_DATACONN, buf); // 发送150代码（打开文件成功，数据传输链接创建成功）

	unsigned long long total_size = sbuf.st_size; // 文件总大小，此处应该改为：unsigned long long total_size = (unsigned long long)sbuf.st_size;

	//断点续载
	unsigned long long offset = sess->restart_pos;
	sess->restart_pos = 0;
	if(offset >= total_size) // 如果偏移量大于等于文件的总大小，则提示传输完毕
	{
		ftp_reply(sess, FTP_TRANSFEROK, "Transfer complete."); // 发送226代码（传输已完成）
	}
	else
	{
		...
		else
		{
			...
			total_size -= offset; // 续传时，文件的剩余大小

			//登记文件开始下载的时间
			sess->transfer_start_sec = get_time_sec(); // 记录秒
			sess->transfer_start_usec = get_time_usec(); // 记录微秒
...
static void do_stor(session_t *sess) // 上传文件
{
	...
	if(sess->data_fd != -1)
	{
		...
		// 上传数据完毕后，会关闭控制连接闹钟，所以需要重新开启控制连接断开
	start_cmdio_alarm();
}
//断点续传（上传和下载）
static void do_rest(session_t *sess)
{
	...
	char text[MAX_BUFFER_SIZE] = {0}; // 
	sprintf(text, "Restart position accepted (%llu).", sess->restart_pos); // 组合信息
